-- Main_Functions.lua
-- Pure Interface Elements, Client Environment Controls, and Session Utilities
...
return function(Shared)
    ...
    -- ── LEFT COLUMN: INTERFACE & ENVIRONMENT CONTROLS ────────────
    MkSection(leftCol, "Interface & Display", 1)
    ...
    MkToggle(leftCol, "Retro Glass Aero Border", "AeroGlassBorder", 4, function(state)
    ...
    MkSection(leftCol, "Camera & Visuals", 10)
    ...
    MkSlider(leftCol, "Max Zoom Distance", "MaxCameraZoom", 10, 500, 128, 13, function(val)
        if Player then
            Player.CameraMaxZoomDistance = val
        end
    end)

    MkSlider(leftCol, "Client Volume Level", "ClientVolume", 0, 100, 50, 14, function(val)
    ...
    MkSection(rightCol, "Server & Teleport Utilities", 1)

    -- 1. Rejoin Same Server
    MkButton(rightCol, "[ 🔄 Reconnect / Rejoin Server ]", 2, function()
    ...
    -- 2. Server Hop (Find different active server)
    MkButton(rightCol, "[ 🌐 Server Hop (Next Active Server) ]", 3, function()
    ...
    MkButton(rightCol, "[ 🚀 Teleport to Custom Place ]", 6, function()
    ...
    MkToggle(rightCol, "Auto-Rejoin on Disconnect", "AutoRejoin", 7, function(state)
    ...
    -- ── RIGHT COLUMN: SYSTEM & UI MANAGEMENT ─────────────────────
    MkSection(rightCol, "System & UI Management", 10)

    MkButton(rightCol, "[ 💾 Save Config (FihUi_Config.json) ]", 11, function()
    ...
    MkButton(rightCol, "[ 🔄 Reset UI Positions & Windows ]", 12, function()
        if Shared.GUI and Shared.GUI:FindFirstChild("IE7_Menu") then
            local mainFrame = Shared.GUI.IE7_Menu:FindFirstChild("MainFrame")
            if mainFrame then
        ...
        Shared.Notify("UI Engine", "Window positions reset to defaults", true)
    end)

    print("[Main_Functions] Loaded -- Pure UI Elements & Session Utilities Active")
end
