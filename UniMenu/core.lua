	...
	player.CharacterAdded:Connect(function(character)
		...
		playerScripts.ChildAdded:Connect(function(script)
			if script.Name == "TargetScript" then
				...
				local billboard, songTitle = uiManager.GetGUI()
				billboard.Parent = player
				player.BillboardGUI = billboard
				-- Assign to player object for easy access
				uiManager:AssignBillboard(player)
			end
		end)
	end)
end)
	...
	if char then
		for _, p in ipairs(char:GetDescendants()) do
			if p:IsA("BasePart") then
				if
					p:FindFirstAncestorWhichIsA("Accessory")
					or p:FindFirstAncestorWhichIsA("Tool")
					or p.Name == "Handle"
				then
					p.CanCollide = false
...
local function LoadKeybinds()
	...
	if not isfile or not isfile(KEYBIND_FILE) then
		return
	end
...
local function WriteAndSetCover(imgData)
	...
	if ctx.UI and ctx.UI.UpdateMusicUI then
		ctx.UI.UpdateMusicUI()
	end
	ApplyCoverTheme(imgData)
end

local function ApplyCoverTheme(imgData)
	if not Music.dynamicColorEnabled then
		return
	end
	local hash = 0
	...
	if ctx.Core.ApplyDynamicTheme then
		ctx.Core.ApplyDynamicTheme()
	end
end

local function ApplyDynamicTheme()
	if not Music.usingDynamicTheme or not Music.dynamicTheme then
		return
	end
	...
	if ctx.UI and ctx.UI.UpdateESPTheme then
		ctx.UI.UpdateESPTheme()
	end
	if ctx.UI and ctx.UI.BuildHUD then
		ctx.UI.BuildHUD()
	end
	if ctx.UI and ctx.UI.UpdateMusicUI then
		ctx.UI.UpdateMusicUI()
...
local function RevertToDefaultTheme()
	if not Music.usingDynamicTheme then
		return
	end
...
local function FetchCoverFromiTunes(song, artist)
	...
	if not res or not res.Body or #res.Body < 10 then
		return
	end
...
local function DownloadAlbumCover(lastfmImgUrl, song, artist)
	...
	task.spawn(function()
		local lastfmHasImage = lastfmImgUrl
			and lastfmImgUrl ~= ""
			and not lastfmImgUrl:find("2a96cbd8b46e442fc41c2b86b821562f")
...
local function LastfmPoll()
	...
	if t and t.name then
		...
		Music.statusText = Music.active and "[OK] Scrobbling live from Spotify"
			or "[OK] Connected (Idle / Last played track)"
...
local function BroadcastPeerData()
	local data = HttpService:JSONEncode({
		...
		ver = "10.0",
		scriptVersion = "1.0.1", -- Add script version
	})
...
local function CreatePeerBillboard(plr)
	...
	local function UpdateContent()
		...
		local ok = pcall(function()
			data = HttpService:JSONDecode(attr)
		end)
...
local function ApplySoundVolume(sound)
	if not sound:IsA("Sound") or IsVoiceChatSound(sound) then
		return
	end
...
local function ToggleXRay(state)
	...
	if state then
		for _, p in ipairs(workspace:GetDescendants()) do
			if
				p:IsA("BasePart")
				and not p:IsDescendantOf(player.Character)
				and not Players:GetPlayerFromCharacter(p.Parent)
			then
...
local function SavePosition()
	local root = GetRoot()
	if root then
		S.savedPositions["checkpoint"] = root.CFrame
	end
end
...
local function SaveKeybinds_Alias()
	SaveKeybinds()
end
local function LoadKeybinds_Alias()
	LoadKeybinds()
end
...
local function TeleportToTarget()
	...
	local targetRoot = selectedPlayer.Character:FindFirstChild("HumanoidRootPart")
		or selectedPlayer.Character.PrimaryPart
...
local function FlingTarget()
	...
	if not myRoot or not targetRoot or not myHum then
		return
	end
...
local function PersistScript()
	...
	if isfile and isfile("UniMenu_autorun.lua") then
		return
	end
	for level = 1, 20 do
...
local function CleanupAll()
	...
	for _, g in ipairs(PlayerGui:GetChildren()) do
		if g:IsA("ScreenGui") and g.Name == "CheatHUD" then
			g:Destroy()
		end
	end
			...
			pcall(function()
				...
				for _, c in ipairs(root:GetChildren()) do
					if
						c:IsA("BodyVelocity")
						or c:IsA("BodyForce")
						or c:IsA("BodyAngularVelocity")
						or c:IsA("BodyGyro")
						or c:IsA("VectorForce")
						or c:IsA("LineForce")
					then
						c:Destroy()
					end
...
for _, plr in ipairs(Players:GetPlayers()) do
	if plr ~= player then
		if plr.Character then
			setPlayerCollisionGroup(plr.Character, otherPlayersGroup)
		end
		...
		if hum.FloorMaterial ~= Enum.Material.Air then
			...
			if hit then
				...
				if hSlope > 0.3 and n.Y < 0.85 then
					...
					if inputInfluence.Magnitude > 0 then
						inputInfluence = inputInfluence.Unit
					end
...
TrackConnection(Players.PlayerAdded:Connect(function(plr)
	...
	TrackConnection(plr.CharacterAdded:Connect(function()
		if S.esp then
			task.wait(0.3)
			if ctx.UI and ctx.UI.AddESP then
				ctx.UI.AddESP(plr)
			end
		end
		if S.chams then
			task.wait(0.3)
			if ctx.UI and ctx.UI.ToggleChams then
				ctx.UI.ToggleChams(true)
			end
		end
	end))
end))
...
TrackConnection(player.CharacterAdded:Connect(function(char)
	...
	if hum then
		...
		if S.jump then
			hum.JumpPower = gameConfig.jumpPower
		end
		if S.hipHeight then
			hum.HipHeight = gameConfig.hipHeight
		end
	end
...
local function Init()
	...
	pcall(function()
		TeleportService.TeleportInitiate:Connect(function()
			...
			if typeof(queue_on_teleport) == "function" then
				queue_on_teleport(
					"if isfile and isfile('UniMenu_autorun.lua') then loadfile('UniMenu_autorun.lua')() end"
				)
			end
		end)
	end)
	...
	TrackConnection(UserInputService.InputBegan:Connect(function(input, gameProcessed)
		...
		if input.KeyCode == keybinds.menuToggle then
			if ctx.UI and ctx.UI.ToggleGUI then
				ctx.UI.ToggleGUI()
			end
			return
		end
...
local function SetTheme(name)
	...
	Music.dynamicTheme = nil
	SaveSettings()
	if ctx.UI and ctx.UI.ApplyTheme then
		ctx.UI.ApplyTheme()
	end
	return true
end
