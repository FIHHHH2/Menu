...
local builtInFeatures = {
  ...
  Server = {
    ...
    { name = "Rejoin",      desc = "Rejoin current server",           isButton = true, action = function() ctx.Services
          .TeleportService:Teleport(game.PlaceId, ctx.Services.Players.LocalPlayer) end },
    { name = "Server Hop",  desc = "Join a different server",         isButton = true, action = function() ctx.Core
          .ServerHop() end },
    { name = "Copy Job ID", desc = "Copy current JobId to clipboard", isButton = true, action = function() if setclipboard then
        setclipboard(game.JobId)
        ctx.Core.ShowNotification("Job ID copied")
      end end },
    { isSection = true,     name = "Utilities" },
    { name = "Anti-AFK",    desc = "Prevent idle kick",               isToggle = true, get = function() return S.antiAfk end,                                                                               toggle = function(
        state) S.antiAfk = state end },
    { name = "Chat Spy",    desc = "See all chat messages",           isToggle = true, get = function() return S.chatSpy end,                                                                               toggle = function(
        state) S.chatSpy = state end },
  },
  Themes = {
  ...
  Config = {
    ...
    { name = "Save Settings",     desc = "Save current config to file", isButton = true, action = function()
      ctx.Core.SaveSettings()
      ctx.Core.ShowNotification("Settings saved")
    end },
    { name = "Load Settings",     desc = "Load config from file",       isButton = true, action = function()
      ctx.Core.LoadSettings()
      ctx.Core.ShowNotification("Settings loaded")
    end },
    { name = "Reset to Defaults", desc = "Reset all settings",          isButton = true, action = function()
      ctx.Core.ResetSettings()
      ctx.Core.ShowNotification("Settings reset")
    end },
  },
  HUD = {
  ...
  Keybinds = {
    ...
    { name = "Reset All Keybinds", desc = "Clear all custom keybinds", isButton = true, action = function()
      ctx.Core.ResetKeybinds()
      ctx.Core.ShowNotification("Keybinds reset")
    end },
  },
  Music = {
