    ...
    local function sendBridgeCommand(action)
        ...
        pcall(function()
            if typeof(writefile) == "function" or (getgenv and typeof(getgenv().writefile) == "function") then
                local wf = writefile or getgenv().writefile
                wf("fih_bridge_cmd.txt", action .. ":" .. tostring(os.time()) .. tostring(math.random(100, 999)))
                wroteFile = true
            end
        end)
    ...
    local function getBridgeTrack()
        ...
        pcall(function()
            if (typeof(readfile) == "function" or (getgenv and typeof(getgenv().readfile) == "function"))
               and (typeof(isfile) == "function" or (getgenv and typeof(getgenv().isfile) == "function")) then
                local rf = readfile or getgenv().readfile
                local exists = isfile or getgenv().isfile
                if exists("fih_bridge_state.json") then
                    local content = rf("fih_bridge_state.json")
                    if content and #content > 5 then
                        local okJ, decoded = pcall(function() return Http:JSONDecode(content) end)
        ...
        if not data or not data.name or data.name == "" or data.name == "Local Audio Session" then return nil end
